...
async def register_badges(client, tree, data_dir):
    cache_path = os.path.join(data_dir, "twitch_badges_cache.json")
    state = _load(cache_path, {"hash":"", "badges":[]})
    ...
    grp = app_commands.Group(name="badges", description="Twitch badges")

    @grp.command(name="latest", description="Show latest Twitch badges")
