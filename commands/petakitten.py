...
def resolve_tier(count: int):
    for index, (min_count, name, color) in enumerate(KITTY_TIERS, start=1):
        if count >= min_count:
            ...
            return {"tier": name, "tier_number": tier_number, "color": color}
    return None
...
def register(bot):
    ...
    async def pet_kitten(interaction: discord.Interaction):
        ...
        if achievement_unlocked:
            ...
            await interaction.channel.send(f"🏆 **Achievement unlocked:** {achievement_unlocked}\n{interaction.user.mention}")

        # Embed
        embed = discord.Embed(title="🐱 Pet a Kitten", description=random_text, color=tier_data["color"])
        embed.add_field(name="🐱 Your Interaction", value=f"**{user_count}**", inline=True)
        embed.add_field(name="🐾 Server's Help", value=f"**{server_total}**", inline=True)
        embed.add_field(name="🦁 Tier", value=f"**{ROMAN[tier_data['tier_number']-1]}**", inline=False)
    ...
    async def pet_error(interaction: discord.Interaction, error):
        if isinstance(error, app_commands.errors.CommandOnCooldown):
            await interaction.response.send_message(f"🐾 The kittens are resting. Try again in {int(error.retry_after)} seconds.", ephemeral=True)
        else:
