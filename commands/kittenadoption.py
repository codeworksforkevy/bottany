    ...
    async def adopt(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            async with self.bot.db.acquire() as conn:
                # Multiple kittens allowed, no conflict check
                await conn.execute("""
        ...
        embed.add_field(name="Kitten Name", value=self.kitten_name, inline=False)
        embed.set_footer(text=f"{interaction.user.display_name} adopted a kitten")
        ...
        if random.randint(1,100) <= 8:
            ...
            await interaction.channel.send(f"🌍 **Global Kitten Event**\n\n{event}")

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.grey)
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id:
            return
        await interaction.response.edit_message(content="Kitten adoption cancelled.", embed=None, view=None)
...
def register(bot):
    ...
    async def kittenadoption(interaction: discord.Interaction, name: str):
        ...
        view = KittenAdoptView(bot, interaction.guild.id, interaction.user.id, name)
