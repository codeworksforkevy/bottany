...
# --- Gaming-only filtering (avoid sports/news noise) ---
_POSITIVE_KW = [
    "video game", "videogame", "gaming", "game studio", "game developer", "pc game",
    "steam", "epic games", "gog", "playstation", "xbox", "nintendo", "switch",
    "console", "gpu", "amd", "nvidia", "intel", "unity", "unreal engine",
]
_NEGATIVE_KW = [
    "football", "soccer", "nba", "nfl", "mlb", "nhl", "cricket", "tennis",
    "formula 1", "f1", "ufc", "boxing", "olympic", "premier league", "la liga",
    "transfer", "match", "goal", "scoreline",
]

def _looks_like_gaming(title: str, desc: str) -> bool:
    t = f"{title} {desc}".lower()
    if any(k in t for k in _NEGATIVE_KW):
        return False
    return any(k in t for k in _POSITIVE_KW)
