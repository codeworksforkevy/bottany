...
def _load(data_dir: str) -> Dict[str, Any]:
    default = {
        "version": 1,
        "enabled": True,
        "keywords": ["free nitro", "steam gift", "airdrop"],
        "regex": []
    }
...
class ModerationGroup(app_commands.Group):
    def __init__(self, bot: discord.Client, data_dir: str):
        super().__init__(
            name="moderation",
            description="Moderation tools (keyword/spam detection)"
        )
        self._bot = bot
    ...
    async def spam_status(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Enabled",
            value=str(bool(obj.get("enabled", True))),
            inline=True
        )
        embed.add_field(
            name="Keywords",
            value=str(len(obj.get("keywords", []) or [])),
            inline=True
        )
        embed.add_field(
            name="Regex",
            value=str(len(obj.get("regex", []) or [])),
            inline=True
        )
    ...
    @app_commands.command(
        name="spam_enable",
        description="Admin: enable/disable spam filter"
    )
    ...
    @app_commands.command(
        name="spam_add",
        description="Admin: add keyword to spam filter"
    )
    ...
    @app_commands.command(
        name="spam_remove",
        description="Admin: remove keyword from spam filter"
    )
    ...
    async def spam_remove(
        ...
        obj = _load(self._data_dir)
        obj["keywords"] = [
            x for x in (obj.get("keywords", []) or [])
            if str(x).lower() != k
        ]
    ...
    @app_commands.command(
        name="spam_list",
        description="List spam keywords"
    )
        ...
        if hit:
            ...
            try:
                await message.channel.send(
                    ...
                    "Your message was removed by the spam filter."
                )
            except Exception:
                pass
