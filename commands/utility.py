...
class UtilityGroup(app_commands.Group):
    ...
    async def ping(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            f"Pong. Latency: {int(self._bot.latency * 1000)}ms"
        )
    ...
    async def poll(
        ...
        desc = "\n".join(
            [f"{emoji[i]} {opts[i]}" for i in range(len(opts))]
        )
    ...
    @app_commands.command(
        name="remind",
        description="Set a simple reminder (minutes)"
    )
    @app_commands.describe(
...
async def register_utility(bot: discord.Client, data_dir: str) -> None:
    ...
    if not hasattr(bot, "_utility_reminders_task"):
        ...
        bot._utility_reminders_task = asyncio.create_task(
            _reminder_loop()
        )
