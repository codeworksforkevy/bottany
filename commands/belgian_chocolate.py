...
def _load_dataset(data_dir: str) -> List[Dict[str, Any]]:
    ...
    for item in merged:
        ...
        if key not in seen:
            ...
            # Already seen — enrich existing entry with any fields it's missing.
            # This ensures professional JSON fields (image_url, image_credit,
            # notes, certifications, production_model…) are never lost even
            # when the desserts dataset loaded the same brand first.
            target = index[key]
