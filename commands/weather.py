...
def register_weather(bot: discord.Client, DATA_DIR: str) -> None:
    ...
    weather_group = app_commands.Group(name="weather", description="Weather information")
    ...
    async def today(interaction: discord.Interaction, city: str):
        ...
        try:
            ...
            await interaction.followup.send("Weather request timed out. Please try again.", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"Weather error: `{type(e).__name__}: {e}`", ephemeral=True)
