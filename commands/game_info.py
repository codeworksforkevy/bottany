    ...
    async def info(self, interaction: discord.Interaction, query: str):
        ...
        if not q:
            await interaction.response.send_message(
                "Provide a Steam appid (number) or a game name.",
                ephemeral=True
            )
            return

        appid = int(q) if q.isdigit() else None
        ...
        if appid:
            ...
            if data:
                ...
                genres = ", ".join(
                    [g.get("description") for g in (data.get("genres") or []) if g.get("description")][:4]
                )
                if genres:
        ...
        embed = discord.Embed(
            title=title[:256],
            description=(desc or "").strip()[:4096]
        )
        ...
        embed.set_footer(
            text="Steam details via Steam store API when appid is provided. Other links are official search entry points."
        )
...
async def register(bot: discord.Client, data_dir: str) -> None:
    ...
    bot.tree.add_command(GameInfoGroup(), guild=guild)
