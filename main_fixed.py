...
@weather_group.command(name="city", description="Weather for today by city (alias of /weather today).")
@app_commands.describe(city="City name (e.g., Berlin, Ankara, Tokyo)", source="auto | metno | openmeteo | dwd")
async def weather_city(interaction: discord.Interaction, city: str, source: str = "auto"):
    await weather_today(interaction, city, source)
...
async def weather_forecast(interaction: discord.Interaction, lat: float, lon: float):
    ...
    base_url = api.get("base_url", "https://api.met.no/weatherapi/locationforecast/2.0/compact")
    if not _allowed_domain("weather", base_url):
        await interaction.response.send_message("Weather API domain is not allowed by governance.")
        return
    ...
    try:
        ...
        hum = details.get("relative_humidity")
        pres = details.get("air_pressure_at_sea_level")

        embed = discord.Embed(title="Weather forecast (official data)")
        embed.add_field(name="Coordinates", value=f"{lat}, {lon}", inline=True)
        if ts:
            embed.add_field(name="Time (UTC)", value=ts, inline=True)
        if air is not None:
            embed.add_field(name="Air temperature", value=f"{air} °C", inline=True)
        if wind is not None:
            embed.add_field(name="Wind speed", value=f"{wind} m/s", inline=True)
        if hum is not None:
            embed.add_field(name="Relative humidity", value=f"{hum} %", inline=True)
        if pres is not None:
...
async def freegames_now(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="Free games — Official sources")
    if epic_items:
        embed.description = "Current Epic promotions (best-effort):\n" + "\n".join([f"• **{t}**" for t,_ in epic_items[:5]])
    else:
        embed.description = "Use the buttons below to open official free-games pages."
