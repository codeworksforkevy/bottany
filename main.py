...
from freegames_logic import classify_games
from post_builder import build_weekly_post
from sanity_check import run_sanity_check
from weekly_freegames_updater import update_weekly_freegames_cache
import inspect
...
# -------------------------
# Weekly Free Games (auto-post) — safe, policy-based
# -------------------------
# This scheduler reads a weekly cache (produced by your /freegames command module or an external updater)
# and posts only if the sanity-check passes.
#
# Cache file format (list of entries):
# [
#   {"platform":"epic","title":"Rustler - Grand Theft Horse","url":"...","price":0,"free_to_keep":true,"claim_until":"2026-01-29T16:00:00.000Z"},
#   {"platform":"gog","title":"Ultima IV","url":"...","price":0,"free_to_keep":true,"claim_until":null}
# ]
FREEGAMES_CACHE_PATH = os.path.join(DATA_DIR, os.getenv("WEEKLY_FREEGAMES_CACHE_FILE", "weekly_freegames_cache.json"))


# Cache updater schedule (UTC). This keeps weekly_freegames_cache.json fresh automatically.
FREEGAMES_CACHE_UPDATE_HOURS = int(os.getenv("FREEGAMES_CACHE_UPDATE_HOURS", "6"))  # update every N hours

async def refresh_weekly_freegames_cache() -> dict:
    """Fetch sources and write weekly cache file. Returns a small report dict."""
    try:
        report = await update_weekly_freegames_cache(FREEGAMES_CACHE_PATH)
        logger.info("[WeeklyFreeGames] Cache refresh ok | %s", report)
        return report or {}
    except Exception as e:
        logger.warning("[WeeklyFreeGames] Cache refresh failed: %s", e)
        return {"ok": False, "error": str(e)}

@tasks.loop(hours=1)
async def weekly_freegames_cache_updater():
    # Update every FREEGAMES_CACHE_UPDATE_HOURS (hourly loop with modulo check).
    tz = get_tz()
    now = datetime.now(tz) if tz else datetime.now()
    if (now.hour % max(1, FREEGAMES_CACHE_UPDATE_HOURS)) != 0:
        return
    await refresh_weekly_freegames_cache()


# Weekly schedule defaults (UTC). Configure via env vars on Railway if you want.
FREEGAMES_POST_DOW = int(os.getenv("FREEGAMES_POST_DOW", "1"))         # 1=Monday ... 7=Sunday (ISO)
FREEGAMES_POST_HOUR = int(os.getenv("FREEGAMES_POST_HOUR", "10"))      # 0-23
FREEGAMES_POST_MINUTE = int(os.getenv("FREEGAMES_POST_MINUTE", "0"))   # 0-59

def load_weekly_freegames_cache() -> list[dict]:
    try:
        if not os.path.exists(FREEGAMES_CACHE_PATH):
            return []
        with open(FREEGAMES_CACHE_PATH, "r", encoding="utf-8") as f:
            obj = json.load(f)
        return obj if isinstance(obj, list) else []
    except Exception as e:
        logger.warning("Weekly freegames cache load failed: %s", e)
        return []

async def post_weekly_freegames_for_guild(guild: discord.Guild) -> bool:
    # Resolve target channel (reuse channels table)
    chan_id = db_get_channel(guild.id, "freegames")
    if not chan_id:
        return False
    channel = guild.get_channel(chan_id)
    if not isinstance(channel, discord.TextChannel):
    ...
    entries = load_weekly_freegames_cache()
    if not entries:
        logger.info("[WeeklyFreeGames] No cache entries; skipping for guild=%s", guild.id)
        return False

    free_games, discounted_games, rejected_games = classify_games(entries)
    sanity = run_sanity_check(free_games, discounted_games, rejected_games)

    # Sanity-check FAIL -> cancel post (requested behavior)
    should_cancel = (
        sanity.get("free_count", 0) == 0
        or (sanity.get("rejected_count", 0) > 0 and sanity.get("free_count", 0) <= 1)
    )
    logger.info(
        "[WeeklyFreeGames] guild=%s free=%s discounted=%s rejected=%s",
        guild.id,
        sanity.get("free_count", 0),
        sanity.get("discounted_count", 0),
        sanity.get("rejected_count", 0),
    )
    if should_cancel:
        logger.warning("[WeeklyFreeGames] Sanity-check failed -> post cancelled | %s", sanity)
        return False

    post_text = build_weekly_post(free_games, discounted_games)
    await channel.send(post_text)
    return True

@tasks.loop(minutes=1)
async def weekly_freegames_scheduler():
    # Use timezone if available; otherwise system time.
    tz = get_tz()
    now = datetime.now(tz) if tz else datetime.now()
    # ISO weekday: Monday=1 ... Sunday=7
    if now.isoweekday() != FREEGAMES_POST_DOW:
        return
    if now.hour != FREEGAMES_POST_HOUR or now.minute != FREEGAMES_POST_MINUTE:
        return

    for guild in bot.guilds:
        try:
            await post_weekly_freegames_for_guild(guild)
        except Exception as e:
            logger.warning("[WeeklyFreeGames] Post failed for guild %s: %s", guild.id, e)
...
async def on_ready():
    ...
    # Ensure /badges commands are registered once (Twitch badges UX: minimal + paginated)
    if not getattr(bot, "_badges_registered", False):
        try:
            if register_twitch_badges:
                # register_twitch_badges is synchronous; no await required
                register_twitch_badges(bot, DATA_DIR)
...
if not weekly_freegames_scheduler.is_running():
    weekly_freegames_scheduler.start()
@bot.event
