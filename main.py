...
from utils.twitch_badges_and_drops_poller import poll_once
...
# Ensure /twitch badges and drops is registered once
if not getattr(bot, "_twitch_badges_and_drops_registered", False):
    try:
        register_twitch_badges_and_drops(bot, DATA_DIR)
        bot._twitch_badges_and_drops_registered = True
    except Exception as e:
        if "already registered" in str(e).lower():
            bot._twitch_badges_and_drops_registered = True
            logger.warning("Twitch badges and drops command group was already registered; continuing.")
        else:
            logger.warning("Twitch badges and drops module registration failed: %s", e)
...
# Start Twitch badges & drops poller loop (single announce channel via Railway variables)
if not getattr(bot, "_twitch_badges_and_drops_poll_started", False):
    bot._twitch_badges_and_drops_poll_started = True

    channel_id = int(os.getenv("TWITCH_ANNOUNCE_CHANNEL_ID", "0") or "0")
    interval = int(os.getenv("TWITCH_POLL_INTERVAL_SECONDS", "900") or "900")
    game_id = (os.getenv("TWITCH_GAME_ID", "") or "").strip() or None

    client_id = (os.getenv("TWITCH_CLIENT_ID", "") or "").strip()
    client_secret = (os.getenv("TWITCH_CLIENT_SECRET", "") or "").strip()

    announce_channel = bot.get_channel(channel_id) if channel_id else None

    async def _twitch_poll_loop():
        async with aiohttp.ClientSession() as session:
            while True:
                try:
                    await poll_once(
                    ...
                    logger.warning("Twitch poll loop iteration failed: %s", e)
                await asyncio.sleep(max(60, interval))

    bot.loop.create_task(_twitch_poll_loop())
