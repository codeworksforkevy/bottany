...
try:
    from commands.twitch_stream import register_twitch_stream
except Exception:
    register_twitch_stream = None
try:
    from commands.utility import register_utility
except Exception:
    register_utility = None
try:
    from commands.game_info import register_game_info
except Exception:
    register_game_info = None
try:
    from commands.moderation_spam import register_moderation_spam
except Exception:
    register_moderation_spam = None
...
# await register_drawing(bot, DATA_DIR)
...
async def on_ready():
    ...
    if not getattr(bot, "_free_games_registered", False):
        try:
            await register_free_games(bot, DATA_DIR)
    ...
    # Ensure /twitch commands are registered once
    if register_twitch_stream and not getattr(bot, "_twitch_stream_registered", False):
        try:
            await register_twitch_stream(bot, DATA_DIR)
            bot._twitch_stream_registered = True
        except Exception as e:
            if "already registered" in str(e).lower():
                bot._twitch_stream_registered = True
                logger.warning("Twitch command group was already registered; continuing.")
            else:
                logger.warning("Twitch module registration failed: %s", e)

    # Ensure /utility commands are registered once
    if register_utility and not getattr(bot, "_utility_registered", False):
        try:
            register_utility(bot, DATA_DIR)
            bot._utility_registered = True
        except Exception as e:
            if "already registered" in str(e).lower():
                bot._utility_registered = True
                logger.warning("Utility command group was already registered; continuing.")
            else:
                logger.warning("Utility module registration failed: %s", e)

    # Ensure /gameinfo commands are registered once
    if register_game_info and not getattr(bot, "_game_info_registered", False):
        try:
            await register_game_info(bot, DATA_DIR)
            bot._game_info_registered = True
        except Exception as e:
            if "already registered" in str(e).lower():
                bot._game_info_registered = True
                logger.warning("Game info command group was already registered; continuing.")
            else:
                logger.warning("Game info module registration failed: %s", e)

    # Ensure spam/keyword moderation is registered once
    if register_moderation_spam and not getattr(bot, "_moderation_spam_registered", False):
        try:
            register_moderation_spam(bot, DATA_DIR)
            bot._moderation_spam_registered = True
        except Exception as e:
            if "already registered" in str(e).lower():
                bot._moderation_spam_registered = True
                logger.warning("Moderation spam module was already registered; continuing.")
            else:
                logger.warning("Moderation spam module registration failed: %s", e)

    # Start healthcheck server once (Railway)
    if not getattr(bot, "_healthcheck_started", False):
        try:
            asyncio.create_task(start_healthcheck_server())
            bot._healthcheck_started = True
        except Exception as e:
            logger.warning("Healthcheck server start failed: %s", e)
    ...
    # --- Slash command sync ---
    # DEV_GUILD_ID enables faster 'instant' syncing to a single test server.
    # If DEV_GUILD_ID is set, we sync to that guild; otherwise we sync globally.
    try:
        ...
        if dev_guild_id:
            ...
            logger.info("Synced %s command(s) to DEV_GUILD_ID=%s. Logged in as %s", len(synced), gid, bot.user)
        else:
...
async def weekly_freegames_task():
    ...
    try:
        ...
        embed = discord.Embed(title="Weekly Free Games and Selected Discounts — Official sources", description="Use the buttons to open official pages. (Epic list is best-effort.)")
        ...
        logger.info("Posted Weekly Free Games and Selected Discounts update to channel %s", channel_id)
    except Exception as e:
        logger.warning("Weekly Free Games and Selected Discounts post failed: %s", e)
