...
try:
    ...
    register_belgian_chocolate = None
    ...
    if not getattr(bot, "_belgium_beverages_registered", False):
        try:
            if register_belgium_beverages:
                await register_belgium_beverages(bot, DATA_DIR)
...
# Ensure /belgium chocolate commands are registered once
    if not getattr(bot, "_belgian_chocolate_registered", False):
        try:
            if register_belgian_chocolate:
                await register_belgian_chocolate(bot, DATA_DIR)
            bot._belgian_chocolate_registered = True
        except Exception as e:
            if "already" in str(e).lower():
                bot._belgian_chocolate_registered = True
                logger.warning("Belgian chocolate commands were already registered; continuing.")
            else:
                logger.warning("Belgian chocolate registration failed: %s", e)
