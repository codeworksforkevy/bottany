...
try:
    from commands.theory import register_theory
except Exception:
    register_theory = None
...
async def on_ready():
    ...
    # Ensure /theory commands are registered once
    if register_theory and not getattr(bot, '_theory_registered', False):
        try:
            await register_theory(bot, DATA_DIR)
            bot._theory_registered = True
        except Exception as e:
            if 'already registered' in str(e).lower():
                bot._theory_registered = True
                logger.warning('Theory command group was already registered; continuing.')
            else:
                logger.warning('Theory module registration failed: %s', e)
...
async def weekly_freegames_task():
    ...
    try:
        ...
        embed = discord.Embed(title="Weekly Free Games and Selected Discounts — Official sources", description="Official entry points for free-to-keep games and selected discounted deals. (Epic list is best-effort.)")
