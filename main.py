...
try:
    from commands.manga_learn import register_manga_learn
except Exception:
    register_manga_learn = None
...
async def on_ready():
    ...
    # Ensure /manga commands are registered once
    if not getattr(bot, "_manga_learn_registered", False):
        try:
            if register_manga_learn is not None:
                register_manga_learn(bot, DATA_DIR)
            bot._manga_learn_registered = True
        except Exception as e:
            if "already registered" in str(e).lower():
                bot._manga_learn_registered = True
                logger.warning("Manga command group was already registered; continuing.")
            else:
                logger.warning("Manga module registration failed: %s", e)
