...
# (Weather commands moved to commands/weather.py)
...
@freegames_group.command(name="set_channel", description="Admin: set the channel for weekly free-games announcements.")
@app_commands.checks.has_permissions(manage_guild=True)
@app_commands.describe(channel="Channel to post weekly free-games updates")
async def set_freegames_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    if not await enforce_rate_limit(interaction, "freegames_set_channel", cooldown_seconds=5):
        return
    BOT_CFG["freegames_announce_channel_id"] = channel.id
    save_bot_cfg()
    await interaction.response.send_message(f"Weekly free-games announcements will be posted in {channel.mention}.", ephemeral=True)


def _epic_free_games():
    url = "https://store-site-backend-static.ak.epicgames.com/freeGamesPromotions?locale=en-US&country=US&allowCountries=US"
    if not _allowed_domain("gaming_deals", url):
        return []
    headers = {"User-Agent": "Bottany/1.0 (+https://railway.app)"}
    r = requests.get(url, headers=headers, timeout=20)
    r.raise_for_status()
    js = r.json()
    elements = (((js.get("data", {}) or {}).get("Catalog", {}) or {}).get("searchStore", {}) or {}).get("elements", []) or []
    out = []
    for e in elements:
        ...
        if not offers:
            continue
        title = e.get("title") or e.get("productSlug") or "Epic free game"
        slug = e.get("productSlug") or ""
        page = f"https://store.epicgames.com/en-US/p/{slug}" if slug else "https://store.epicgames.com/free-games"
        if _allowed_domain("gaming_deals", page):
...
async def freegames_now(interaction: discord.Interaction):
    ...
    # Normalize sources: dict -> list[dict], list -> list[dict]
    ...
    embed = discord.Embed(
        ...
        description="Official entry points + best-effort current Epic promotions.",
    )

    if epic_items:
        epic_lines = [f"• {ICON['epic']} [{title}]({url})" for title, url in epic_items[:5]]
        embed.add_field(name="Epic (current promotions)", value="
".join(epic_lines), inline=False)
    else:
        embed.add_field(
            name="Epic (current promotions)",
            value="No items fetched (best-effort). Use the official buttons below.",
            inline=False,
        )
    ...
    class _FGView(discord.ui.View):
        def __init__(self):
            ...
            # Add up to 4 official source buttons
            added = 0
            for s in sources[:6]:
                if added >= 4:
                    break
                ...
                # Prefer explicit url; otherwise fall back to known official entry points
                ...
                # Pick icon based on URL
                label_icon = ICON["default"]
                u = url.lower()
                if "epicgames.com" in u:
                    label_icon = ICON["epic"]
                elif "gog.com" in u:
                    label_icon = ICON["gog"]

                self.add_item(discord.ui.Button(label=f"{label_icon} {name}"[:80], url=url))
                added += 1

            # Add a few direct Epic buttons for convenience
            for title, url in epic_items[:3]:
                if _allowed_domain("gaming_deals", url):
                    self.add_item(discord.ui.Button(label=f"{ICON['epic']} Epic: {title}"[:80], url=url))

    embed.set_footer(
        text="Official-source buttons are allowlist-checked. Weekly announcements post only to the configured #gaming channel."
    )

    await interaction.response.send_message(embed=embed, view=_FGView())

bot.tree.add_command(freegames_group)
